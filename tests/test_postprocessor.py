...
from folioforge.postprocessor.label_filter import LabelConfidenceFilter, LabelFilter
            ...
            items=[
                DocumentEntry(
                    ...
                    layout=[
                        ...
                        Text(bbox=bbox, label=Label.TEXT, confidence=0.4, converted=None),
                    ],
                    converted=None,
                )
            ],
...
def test_label_confidence_filter(documents: list[DocumentReference]):
    filter = LabelConfidenceFilter(label_confidence={Label.TEXT: 0.3, Label.IMAGE: 0.5})
    filtered = list(filter.process(documents, outdir=Path(".")))
    assert filtered
    assert len(filtered) == 1
    assert len(filtered[0].items) == 1
    assert len(filtered[0].items[0].layout) == 3
    assert all(i.confidence >= 0.3 for i in filtered[0].items[0].layout if i.label == Label.TEXT)
